...
namespace urlicht::concepts {
	...
	// Checks if the range gives lvalue reference
	template <typename Rng>
	concept lvalue_range =
			std::ranges::input_range<Rng> && std::is_lvalue_reference_v<std::ranges::range_reference_t<Rng>>;

	// Checks if the range gives rvalue reference
	template <typename Rng>
	concept rvalue_range =
			std::ranges::input_range<Rng> && std::is_rvalue_reference_v<std::ranges::range_reference_t<Rng>>;
	...
	|| (urlicht::concepts::decays_to_ptr<T> && urlicht::concepts::decays_to_ptr<U>);
	...
	concept nothrow_equality_comparable =
		urlicht::concepts::equality_comparable<T, U> &&
		((urlicht::concepts::decays_to_ptr<T> && urlicht::concepts::decays_to_ptr<U>) ||
		noexcept(std::declval<const T&>() == std::declval<const U&>()));

	template <typename T, typename U = T>
	concept nothrow_inequality_comparable =
		urlicht::concepts::inequality_comparable<T, U> &&
		((urlicht::concepts::decays_to_ptr<T> && urlicht::concepts::decays_to_ptr<U>) ||
		noexcept(std::declval<const T&>() != std::declval<const U&>()));

	template <typename T, typename U = T>
	concept nothrow_less_comparable =
		urlicht::concepts::less_comparable<T, U> &&
		((urlicht::concepts::decays_to_ptr<T> && urlicht::concepts::decays_to_ptr<U>) ||
		noexcept(std::declval<const T&>() < std::declval<const U&>()));

	template <typename T, typename U = T>
	concept nothrow_greater_comparable =
		urlicht::concepts::greater_comparable<T, U> &&
		((urlicht::concepts::decays_to_ptr<T> && urlicht::concepts::decays_to_ptr<U>) ||
		noexcept(std::declval<const T&>() > std::declval<const U&>()));
    ...
    concept allocator =
    requires { typename Alloc::value_type; } &&
    requires (Alloc& alloc, typename Alloc::value_type* p, std::size_t n) {
	...
	&&  (!urlicht::concepts::equality_comparable<typename Cont::value_type> || urlicht::concepts::equality_comparable<Cont>)
	...
	template <typename Cont>
	concept has_allocator = requires (const Cont& cont) {
		typename Cont::allocator_type;
		{ cont.get_allocator() } -> std::same_as<typename Cont::allocator_type>;
	};
	...
	// contiguous container, e.g. std::vector, urlicht::container::inplace_vector
	template <typename Cont>
	...
	concept deque =
		urlicht::concepts::random_access_container<Deq>
	&& requires() {
...
#endif //URLICHT_CONCEPTS_CONCEPTS_H
