...
namespace urlicht::container {
     ...
     *        inplace_vector with proper alignment.
     *
...
namespace urlicht {
    ...
    inline constexpr bool is_urlicht_inplace_vector_v =
        container::detail::is_inplace_vector<Cont>::value;
}
...
namespace urlicht::container {
    ...
    private:
        ...
        template <urlicht::concepts::compatible_iterator<T> Iter, std::sentinel_for<Iter> Sent>
        constexpr auto assign_with_sentinel_(Iter first, Sent sentinel)
        noexcept(std::is_nothrow_constructible_v<T, std::iter_reference_t<Iter>> &&
                 std::is_nothrow_assignable_v<T, std::iter_reference_t<Iter>>) {
            const auto old_size = size_;
            size_type cnt{};
        ...
        template <urlicht::concepts::compatible_iterator<T> Iter>
        constexpr void append_with_size_(Iter first, const size_type n)
        noexcept(std::is_nothrow_constructible_v<T, std::iter_reference_t<Iter>>) {
            const auto old_size = size_;
        ...
        template <urlicht::concepts::compatible_iterator<T> Iter, std::sentinel_for<Iter> Sent>
        constexpr auto append_with_sentinel_(Iter first, Sent last)
        noexcept(std::is_nothrow_constructible_v<T, std::iter_reference_t<Iter>>) {
            size_type cnt{};
        ...
        constexpr void fill_with_size_(const size_type n, const T& val)
    ...
    public:
        ...
        requires std::copy_constructible<value_type> {
            append_with_size_(other.data(), other.size());
        }
        ...
        requires std::default_initializable<value_type> : size_(size){
            if (size > static_cast<size_type>(N)) [[unlikely]] {
                throw std::bad_alloc();
            }
            std::uninitialized_value_construct_n(this->data(), size);
        }
        ...
        constexpr inplace_vector(Iter first, Sentinel last) {
            if constexpr (std::forward_iterator<Iter>) {
                ...
                append_with_size_(first, size);
            } else {
                auto [until, size] = append_with_sentinel_(std::move(first), last);
        ...
        // Range constructor
        template <urlicht::concepts::compatible_range<T> Rng>
        requires (!urlicht::is_urlicht_inplace_vector_v<std::remove_cvref_t<Rng>>)
        constexpr explicit inplace_vector(Rng&& rng)
        : inplace_vector(detail::range_begin(std::forward<Rng>(rng)),
        ...
        requires std::is_copy_assignable_v<value_type> {
            ...
            assign_with_size_(other.data(), other.size());
            return *this;
        }
        ...
        requires std::assignable_from<value_type&, const value_type&> &&
            ...
            if (count > old_size) {
                this->fill_with_size_(count - old_size, value);
            } else {
         ...
         */
        template <urlicht::concepts::compatible_iterator<value_type> Iter, std::sentinel_for<Iter> Sentinel>
        ...
        requires std::assignable_from<value_type&, std::iter_reference_t<Iter>> {
            if constexpr (std::forward_iterator<Iter>) {
                ...
                this->assign_with_size_(first, cnt);
            } else {
        ...
        */
        ...
        noexcept(std::is_nothrow_constructible_v<value_type, Args&&...>) {
            auto end_ptr = this->end();
            std::construct_at(end_ptr, std::forward<Args>(args)...);
            ++this->size_;
            return *end_ptr;
        }
        ...
        noexcept(std::is_nothrow_move_constructible_v<value_type>) {
            auto* end_ptr = this->end();
            std::construct_at(end_ptr, std::move(val));
            ++this->size_;
            return *end_ptr;
        }
        ...
        noexcept(std::is_nothrow_constructible_v<value_type, std::iter_reference_t<Iter>>) {
            if constexpr (std::forward_iterator<Iter>) {
                const auto size = std::ranges::distance(first, last);
                this->append_with_size_(first, size);
            } else {
                this->append_with_sentinel_(std::move(first), last);
            }
        }
        ...
        noexcept(std::is_nothrow_constructible_v<value_type, std::iter_reference_t<Iter>>) {
            if constexpr (std::forward_iterator<Iter>) {
                ...
                this->append_with_size_(first, k);
        ...
        constexpr void append_range(Iter first, Sentinel last) {
            if constexpr (std::forward_iterator<Iter>) {
                ...
                this->append_with_size_(first, cnt);
            } else {
        ...
        requires std::copy_constructible<value_type> && std::is_move_assignable_v<value_type> {
            ...
            if (curr_size + cnt > max_size()) [[unlikely]] {
                throw std::bad_alloc{};
            }
            auto curr_begin = this->begin();
            auto curr_end = this->end();

            if (cpos < curr_begin || cpos - curr_begin > curr_size) [[unlikely]] {
                throw std::out_of_range{"inplace_vector::insert()"};
            }
            auto pos = curr_begin + (cpos - curr_begin);
            const auto tail_len = curr_end - pos;
            ...
            if constexpr (std::is_nothrow_constructible_v<value_type, const_reference> &&
                ...
                if (cnt >= tail_len) {
                    ...
                    const auto tail = curr_end - cnt;
                    std::uninitialized_move_n(tail, cnt, curr_end);
                    std::move_backward(pos, tail, curr_end);
                    std::fill_n(pos, cnt, value);
                }
                this->size_ = curr_size + cnt;
            } else {
                this->fill_with_size_(cnt, value);
        ...
        requires std::is_move_assignable_v<value_type> {
            ...
            this->size_ -= static_cast<size_type>(diff);
            return pos;
        }
        ...
        noexcept(urlicht::concepts::nothrow_equality_comparable<value_type>)
        ...
        noexcept((std::three_way_comparable<value_type>
                && urlicht::concepts::nothrow_three_way_comparable<value_type>)
            ||  (!std::three_way_comparable<value_type>
                && urlicht::concepts::nothrow_less_comparable<value_type>))
        requires urlicht::concepts::less_comparable<value_type> {
