...
namespace urlicht::container {
     ...
     *        It implements all methods of std::flat_map with the following extensions/modifications:
     *
     *        1. Template-configurable lower bound algorithm, with the default being about 3x faster than
     *           std::lower_bound (see benchmarks/algorithm/bm_lower_bound.cpp). Configurable algorithm allows for
     *           scenario-specific optimizations, such as using linear search with small containers.
     ...
     *        4. C++20 compatibility. Note that due to language limitations, flat_map::[const_]iterator does not
    ...
    template <
        ...
        typename LowerBoundFn = urlicht::algorithm::lower_bound_fn>
    class flat_map {
        // Type requirements
        static_assert(urlicht::concepts::object<KeyType>, "KeyType must satisfy std::is_object_v");
        static_assert(urlicht::concepts::object<MappedType>, "MappedType must satisfy std::is_object_v");
        static_assert(std::strict_weak_order<KeyCompare, KeyType, KeyType>);
        static_assert(urlicht::concepts::contiguous_container<KeyContainer>);
        static_assert(urlicht::concepts::contiguous_container<MappedContainer>);
        static_assert(detail::lower_bound_for<LowerBoundFn, KeyType, KeyContainer, KeyCompare>);
    ...
    private:
        ...
        public:
            ...
            template <bool RhsConst>
            friend constexpr bool operator==(const self_type& lhs, const iterator_impl<RhsConst>& rhs) noexcept {
                return lhs.key_ == rhs.key_;
            }
    ...
    private:
        ...
        ||  std::strict_weak_order<key_compare, std::remove_cvref_t<Key>, key_type>;

        template <typename Alloc>
        static constexpr bool uses_allocator_ =
            std::uses_allocator_v<key_container_type, Alloc>
        &&  std::uses_allocator_v<mapped_container_type, Alloc>;
        ...
        constexpr std::pair<iterator, bool> emplace_at_pos_(const_iterator pos, Key&& key, VArgs&&... vargs) {
            ...
            try {
                ...
                if constexpr (!detail::emplace_should_have_strong_exception_safety_for<key_type, Key>) {
                    this->clear();
                }
                throw;
            }
            try {
                ...
                if constexpr (detail::emplace_should_have_strong_exception_safety_for<mapped_type, VArgs...>) {
                    try {
        ...
        constexpr size_type append_range_(Rng&& rng) {
            ...
            if constexpr (std::ranges::sized_range<Rng>) {
                ...
                this->reserve(this->size() + append_size);
            }
            for (auto&& v : rng) {
        ...
        constexpr void sort_unique_append_([[maybe_unused]] Rng&& rng) {
            try {
                ...
                // Sort
                detail::zip_view<key_container_type, mapped_container_type> zv{
    ...
    public:
        ...
        explicit constexpr flat_map(const Alloc& alloc)
        : containers_{
        ...
        : containers_{
            std::make_obj_using_allocator<key_container_type>(alloc, std::forward<KeyCont_>(key_cont)),
            std::make_obj_using_allocator<mapped_container_type>(alloc, std::forward<MappedCont_>(mapped_cont))
          },
          comp_{std::forward<KeyComp_>(key_comp)},
          lower_bound_fn_{std::forward<LowerBoundFn_>(lower_bound)} {
            UL_ASSERT(this->keys().size() == this->values().size(), "Different sizes of key/mapped");
        ...
        : containers_{std::forward<KeyCont_>(key_cont), std::forward<MappedCont_>(mapped_cont)},
          ...
          lower_bound_fn_{std::forward<LowerBoundFn_>(lower_bound)} {
            UL_ASSERT(this->keys().size() == this->values().size(), "Different sizes of key/mapped");
            UL_ASSERT((detail::is_sorted_and_unique(this->keys().begin(), this->keys().end(), this->comp_)),
        ...
        template <urlicht::concepts::compatible_iterator<value_type> Iter,
                  ...
                  urlicht::concepts::allocator Alloc>
        requires uses_allocator_<Alloc>
        constexpr flat_map(Iter first, Sentinel last, const Alloc& alloc)
        : flat_map(first, last, key_compare{}, lower_bound_function_type{}, alloc) { }
        ...
        template <urlicht::concepts::compatible_iterator<value_type> Iter,
                  ...
                  urlicht::concepts::allocator Alloc>
        requires uses_allocator_<Alloc>
        constexpr flat_map(urlicht::internal::sorted_t, Iter first, Sentinel last, const Alloc& alloc)
        : flat_map(urlicht::sorted, first, last, key_compare{}, lower_bound_function_type{}, alloc) { }
        ...
        : comp_{std::forward<KeyComp_>(key_comp)},
          lower_bound_fn_{std::forward<LowerBoundFn_>(lower_bound)} {
            UL_ASSERT((detail::is_sorted_and_unique(first, last, this->value_comp())),
        ...
        template <urlicht::concepts::compatible_iterator<value_type> Iter,
                  ...
                  urlicht::concepts::allocator Alloc>
        requires uses_allocator_<Alloc>
        constexpr flat_map(urlicht::internal::sorted_unique_t, Iter first, Sentinel last, const Alloc& alloc)
        : flat_map(urlicht::sorted_unique, first, last, key_compare{}, lower_bound_function_type{}, alloc)
        { }
        ...
        : flat_map(urlicht::sorted_unique, first, last,
                   std::forward<KeyComp_>(key_comp), lower_bound_function_type{}, alloc)
        { }
        ...
        constexpr flat_map(Rng&& rng, const Alloc& alloc)
        ...
        template <urlicht::concepts::compatible_range<value_type> Rng,
                  urlicht::concepts::allocator Alloc>
        requires uses_allocator_<Alloc>
        constexpr flat_map(urlicht::internal::sorted_t, Rng&& rng, const Alloc& alloc)
        ...
        : comp_{std::forward<KeyComp_>(key_comp)},
          lower_bound_fn_{std::forward<LowerBoundFn_>(lower_bound)} {
            UL_ASSERT((detail::is_sorted_and_unique(
        ...
        template <urlicht::concepts::compatible_range<value_type> Rng,
                  urlicht::concepts::allocator Alloc>
        requires uses_allocator_<Alloc>
        constexpr flat_map(urlicht::internal::sorted_unique_t, Rng&& rng, const Alloc& alloc)
        ...
        constexpr flat_map(urlicht::internal::sorted_t,
                           ...
                           KeyComp_&& key_comp = KeyComp_{},
                           LowerBoundFn_&& lower_bound = LowerBoundFn_{})
        : flat_map(urlicht::sorted, init.begin(), init.end(),
                   std::forward<KeyComp_>(key_comp),
                   std::forward<LowerBoundFn_>(lower_bound)) {}
        ...
        constexpr flat_map(urlicht::internal::sorted_unique_t,
                           ...
                           KeyComp_&& key_comp = KeyComp_{},
                           LowerBoundFn_&& lower_bound = LowerBoundFn_{})
        : flat_map(urlicht::sorted_unique, init.begin(), init.end(),
                   std::forward<KeyComp_>(key_comp),
                   std::forward<LowerBoundFn_>(lower_bound)) {}
        ...
        [[nodiscard]] constexpr auto value_comp() const noexcept {
            ...
            noexcept(urlicht::concepts::nothrow_invocable<key_compare, const key_type&, const key_type&>) {
        ...
        constexpr void reserve(size_type n) {
            this->keys_mut_().reserve(n);  // contiguous container must be reservable
            this->values_mut_().reserve(n);
        }

        constexpr void shrink_to_fit() {
            if constexpr (urlicht::concepts::reservable_container<key_container_type>) {
                this->keys_mut_().shrink_to_fit();
            }
            if constexpr (urlicht::concepts::reservable_container<mapped_container_type>) {
        ...
        template <urlicht::concepts::compatible_iterator<value_type> Iter,
                  std::sentinel_for<Iter> Sentinel>
        constexpr void insert(urlicht::internal::sorted_t, Iter first, Sentinel last) {
            sort_unique_append_<true, false>(std::ranges::subrange(first, last));
        }
        ...
        constexpr void insert(urlicht::internal::sorted_t, std::initializer_list<value_type> il) {
        ...
        template <urlicht::concepts::compatible_range<value_type> Rng>
        constexpr void insert_range(urlicht::internal::sorted_t, Rng&& rng) {
            sort_unique_append_<true, false>(std::forward<Rng>(rng));
        }
        ...
        noexcept(std::is_nothrow_move_assignable_v<key_container_type> &&
            ...
            UL_ASSERT(detail::is_sorted_and_unique(key_cont.begin(), key_cont.end(), this->comp_),
                "Keys are not sorted unique");
...
namespace urlicht {
    template <typename T>
    inline constexpr bool is_urlicht_flat_map_v = container::detail::is_flat_map<T>::value;
}
...
namespace urlicht::container {
    ...
    namespace detail {
        ...
        using alloc_vector_t = std::vector<T, typename std::allocator_traits<Alloc>::template rebind_alloc<T>>;
    }

    template <typename KeyCont_, typename MappedCont_,
              typename KeyComp_ = std::less<>,
              typename LowerBound_ = urlicht::algorithm::lower_bound_fn>
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && std::strict_weak_order<KeyComp_, typename KeyCont_::value_type, typename KeyCont_::value_type>
            && detail::lower_bound_for<LowerBound_, typename KeyCont_::value_type, KeyCont_, KeyComp_>
    flat_map(KeyCont_, MappedCont_, KeyComp_ = KeyComp_{}, LowerBound_ = urlicht::algorithm::lower_bound_fn{})
    ...
    template <typename KeyCont_, typename MappedCont_, typename Alloc>
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(KeyCont_, MappedCont_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    std::less<>,
                    KeyCont_,
    ...
    template <typename KeyCont_, typename MappedCont_,
              typename KeyComp_, typename Alloc>
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && std::strict_weak_order<KeyComp_, typename KeyCont_::value_type, typename KeyCont_::value_type>
            && urlicht::concepts::allocator<Alloc>
    flat_map(KeyCont_, MappedCont_, KeyComp_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    template <typename KeyCont_, typename MappedCont_,
              typename KeyComp_, typename LowerBound_, typename Alloc>
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && std::strict_weak_order<KeyComp_, typename KeyCont_::value_type, typename KeyCont_::value_type>
            && detail::lower_bound_for<
                LowerBound_,
                typename KeyCont_::value_type,
                KeyCont_,
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(KeyCont_, MappedCont_, KeyComp_, LowerBound_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && std::strict_weak_order<KeyComp_, typename KeyCont_::value_type, typename KeyCont_::value_type>
            && detail::lower_bound_for<LowerBound_, typename KeyCont_::value_type, KeyCont_, KeyComp_>
    flat_map(urlicht::internal::sorted_t, KeyCont_, MappedCont_,
             KeyComp_ = KeyComp_{}, LowerBound_ = urlicht::algorithm::lower_bound_fn{})
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, KeyCont_, MappedCont_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    std::less<>,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && std::strict_weak_order<KeyComp_, typename KeyCont_::value_type, typename KeyCont_::value_type>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, KeyCont_, MappedCont_, KeyComp_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            ...
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, KeyCont_, MappedCont_, KeyComp_, LowerBound_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && std::strict_weak_order<KeyComp_, typename KeyCont_::value_type, typename KeyCont_::value_type>
            && detail::lower_bound_for<LowerBound_, typename KeyCont_::value_type, KeyCont_, KeyComp_>
    flat_map(urlicht::internal::sorted_unique_t, KeyCont_, MappedCont_, KeyComp_ = KeyComp_{}, LowerBound_ = urlicht::algorithm::lower_bound_fn{})
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, KeyCont_, MappedCont_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    std::less<>,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            && std::strict_weak_order<KeyComp_, typename KeyCont_::value_type, typename KeyCont_::value_type>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, KeyCont_, MappedCont_, KeyComp_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    requires urlicht::concepts::contiguous_container<KeyCont_> && urlicht::concepts::contiguous_container<MappedCont_>
            ...
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, KeyCont_, MappedCont_, KeyComp_, LowerBound_, Alloc)
        -> flat_map<typename KeyCont_::value_type,
                    typename MappedCont_::value_type,
                    KeyComp_,
                    KeyCont_,
    ...
    template <typename Iter, typename Sent,
              typename KeyComp_ = std::less<>,
              typename LowerBoundFn_ = urlicht::algorithm::lower_bound_fn>
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && std::strict_weak_order<
                KeyComp_, detail::iter_key_t<Iter>, detail::iter_key_t<Iter>>
            && detail::lower_bound_for<LowerBoundFn_, detail::iter_key_t<Iter>,
                std::vector<detail::iter_key_t<Iter>>, KeyComp_>
    flat_map(Iter, Sent, KeyComp_ = KeyComp_{}, LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    std::vector<detail::iter_key_t<Iter>>,
                    std::vector<detail::iter_mapped_t<Iter>>,
                    LowerBoundFn_>;

    template <typename Iter, typename Sent, typename Alloc>
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && urlicht::concepts::allocator<Alloc>
    flat_map(Iter, Sent, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    std::less<>,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
                    detail::alloc_vector_t<detail::iter_mapped_t<Iter>, Alloc>>;
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && std::strict_weak_order<
                KeyComp_, detail::iter_key_t<Iter>, detail::iter_key_t<Iter>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(Iter, Sent, KeyComp_, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    template <typename Iter, typename Sent,
              typename KeyComp_, typename LowerBoundFn_, typename Alloc>
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && std::strict_weak_order<
                KeyComp_, detail::iter_key_t<Iter>, detail::iter_key_t<Iter>>
            && detail::lower_bound_for<
                LowerBoundFn_,
                detail::iter_key_t<Iter>,
                detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(Iter, Sent, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            ...
            && detail::lower_bound_for<LowerBoundFn_, detail::iter_key_t<Iter>,
                std::vector<detail::iter_key_t<Iter>>, KeyComp_>
    flat_map(urlicht::internal::sorted_t, Iter, Sent, KeyComp_ = KeyComp_{}, LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    std::vector<detail::iter_key_t<Iter>>,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, Iter, Sent, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    std::less<>,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && std::strict_weak_order<
                KeyComp_, detail::iter_key_t<Iter>, detail::iter_key_t<Iter>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, Iter, Sent, KeyComp_, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            ...
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, Iter, Sent, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            ...
            && detail::lower_bound_for<LowerBoundFn_, detail::iter_key_t<Iter>,
                std::vector<detail::iter_key_t<Iter>>, KeyComp_>
    flat_map(urlicht::internal::sorted_unique_t, Iter, Sent,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, Iter, Sent, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    std::less<>,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            && std::strict_weak_order<
                KeyComp_, detail::iter_key_t<Iter>, detail::iter_key_t<Iter>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, Iter, Sent, KeyComp_, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    requires std::forward_iterator<Iter> && std::sentinel_for<Sent, Iter>
            ...
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, Iter, Sent, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<detail::iter_key_t<Iter>,
                    detail::iter_mapped_t<Iter>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::iter_key_t<Iter>, Alloc>,
    ...
    requires std::ranges::forward_range<Rng>
            && (!urlicht::is_urlicht_flat_map_v<std::remove_cvref_t<Rng>>)
            && std::strict_weak_order<
                KeyComp_, detail::rng_key_t<Rng>, detail::rng_key_t<Rng>>
            && detail::lower_bound_for<LowerBoundFn_, detail::rng_key_t<Rng>,
                std::vector<detail::rng_key_t<Rng>>, KeyComp_>
    flat_map(Rng, KeyComp_ = KeyComp_{}, LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    std::vector<detail::rng_key_t<Rng>>,
                    std::vector<detail::rng_mapped_t<Rng>>,
                    LowerBoundFn_>;
    ...
    flat_map(Rng, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    std::less<>,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
                    detail::alloc_vector_t<detail::rng_mapped_t<Rng>, Alloc>>;

    template <typename Rng, typename KeyComp_, typename Alloc>
    requires std::ranges::forward_range<Rng>
            && (!urlicht::is_urlicht_flat_map_v<std::remove_cvref_t<Rng>>)
            && std::strict_weak_order<
                KeyComp_, detail::rng_key_t<Rng>, detail::rng_key_t<Rng>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(Rng, KeyComp_, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    template <typename Rng, typename KeyComp_, typename LowerBoundFn_, typename Alloc>
    requires std::ranges::forward_range<Rng>
            && (!urlicht::is_urlicht_flat_map_v<std::remove_cvref_t<Rng>>)
            && std::strict_weak_order<
                KeyComp_, detail::rng_key_t<Rng>, detail::rng_key_t<Rng>>
            && detail::lower_bound_for<
                LowerBoundFn_,
                detail::rng_key_t<Rng>,
                detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(Rng, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    requires std::ranges::forward_range<Rng>
            ...
            && detail::lower_bound_for<LowerBoundFn_, detail::rng_key_t<Rng>,
                std::vector<detail::rng_key_t<Rng>>, KeyComp_>
    flat_map(urlicht::internal::sorted_t, Rng, KeyComp_ = KeyComp_{}, LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    std::vector<detail::rng_key_t<Rng>>,
    ...
    requires std::ranges::forward_range<Rng>
            && (!urlicht::is_urlicht_flat_map_v<std::remove_cvref_t<Rng>>)
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, Rng, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    std::less<>,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    requires std::ranges::forward_range<Rng>
            ...
            && std::strict_weak_order<
                KeyComp_, detail::rng_key_t<Rng>, detail::rng_key_t<Rng>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, Rng, KeyComp_, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    requires std::ranges::forward_range<Rng>
            ...
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, Rng, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    requires std::ranges::forward_range<Rng>
            ...
            && detail::lower_bound_for<LowerBoundFn_, detail::rng_key_t<Rng>,
                std::vector<detail::rng_key_t<Rng>>, KeyComp_>
    flat_map(urlicht::internal::sorted_unique_t, Rng, KeyComp_ = KeyComp_{}, LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    std::vector<detail::rng_key_t<Rng>>,
    ...
    requires std::ranges::forward_range<Rng>
            && (!urlicht::is_urlicht_flat_map_v<std::remove_cvref_t<Rng>>)
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, Rng, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    std::less<>,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    requires std::ranges::forward_range<Rng>
            ...
            && std::strict_weak_order<
                KeyComp_, detail::rng_key_t<Rng>, detail::rng_key_t<Rng>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, Rng, KeyComp_, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    requires std::ranges::forward_range<Rng>
            ...
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, Rng, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<detail::rng_key_t<Rng>,
                    detail::rng_mapped_t<Rng>,
                    KeyComp_,
                    detail::alloc_vector_t<detail::rng_key_t<Rng>, Alloc>,
    ...
    template <typename K, typename V,
              typename KeyComp_ = std::less<>,
              typename LowerBoundFn_ = urlicht::algorithm::lower_bound_fn>
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && detail::lower_bound_for<LowerBoundFn_, std::remove_cv_t<K>,
                std::vector<std::remove_cv_t<K>>, KeyComp_>
    flat_map(std::initializer_list<std::pair<K, V>>, KeyComp_ = KeyComp_{}, LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    std::vector<std::remove_cv_t<K>>,
                    std::vector<std::remove_cv_t<V>>,
                    LowerBoundFn_>;

    template <typename K, typename V, typename Alloc>
    requires urlicht::concepts::allocator<Alloc>
    flat_map(std::initializer_list<std::pair<K, V>>, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    std::less<>,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
                    detail::alloc_vector_t<std::remove_cv_t<V>, Alloc>>;

    template <typename K, typename V, typename KeyComp_, typename Alloc>
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(std::initializer_list<std::pair<K, V>>, KeyComp_, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
    ...
    template <typename K, typename V, typename KeyComp_, typename LowerBoundFn_, typename Alloc>
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && detail::lower_bound_for<
                LowerBoundFn_,
                std::remove_cv_t<K>,
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(std::initializer_list<std::pair<K, V>>, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
    ...
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && detail::lower_bound_for<LowerBoundFn_, std::remove_cv_t<K>,
                std::vector<std::remove_cv_t<K>>, KeyComp_>
    flat_map(urlicht::internal::sorted_t, std::initializer_list<std::pair<K, V>>, KeyComp_ = KeyComp_{},
             LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    std::vector<std::remove_cv_t<K>>,
    ...
    template <typename K, typename V, typename Alloc>
    requires urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, std::initializer_list<std::pair<K, V>>, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    std::less<>,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
    ...
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, std::initializer_list<std::pair<K, V>>, KeyComp_, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
    ...
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_t, std::initializer_list<std::pair<K, V>>, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
    ...
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && detail::lower_bound_for<LowerBoundFn_, std::remove_cv_t<K>,
                std::vector<std::remove_cv_t<K>>, KeyComp_>
    flat_map(urlicht::internal::sorted_unique_t, std::initializer_list<std::pair<K, V>>,
             KeyComp_ = KeyComp_{}, LowerBoundFn_ = LowerBoundFn_{})
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
    ...
    template <typename K, typename V, typename Alloc>
    requires urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, std::initializer_list<std::pair<K, V>>, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    std::less<>,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
    ...
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, std::initializer_list<std::pair<K, V>>, KeyComp_, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
    ...
    requires std::strict_weak_order<KeyComp_, std::remove_cv_t<K>, std::remove_cv_t<K>>
            && detail::lower_bound_for<
                ...
                KeyComp_>
            && urlicht::concepts::allocator<Alloc>
    flat_map(urlicht::internal::sorted_unique_t, std::initializer_list<std::pair<K, V>>, KeyComp_, LowerBoundFn_, Alloc)
        -> flat_map<std::remove_cv_t<K>,
                    std::remove_cv_t<V>,
                    KeyComp_,
                    detail::alloc_vector_t<std::remove_cv_t<K>, Alloc>,
...
template <typename KeyType,
          ...
          typename LowerBoundFn,
          typename Alloc>
struct std::uses_allocator<
    urlicht::container::flat_map<KeyType, MappedType, KeyCompare, KeyContainer, MappedContainer, LowerBoundFn>, Alloc>
    : std::bool_constant<
