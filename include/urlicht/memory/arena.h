#ifndef URLICHT_MEMORY_ARENA_H
#define URLICHT_MEMORY_ARENA_H
...
namespace urlicht::memory {
     ...
     *
     *      - When the initial buffer cannot satisfy a request and Opt.use_upstream == true, the
     ...
     *
     * @tparam Opt Compile-time resource options (e.g. whether to fall back to the upstream resource).
     ...
     *         initial buffer is insufficient. Ignored if Opt.use_upstream is false.
     *
     * @note: - This class is typeless and thus incompatible with std::allocator_traits. Use urlicht::memory::resource_view
    ...
    private:
        ...
        UL_NO_UNIQUE_ADDRESS std::conditional_t<
            Opt.use_upstream,
            detail::chunk_footer*,
            std::monostate
        > chunk_footer_{};
    ...
    public:
         ...
         */
        static consteval bool use_upstream() noexcept { return Opt.use_upstream; }

        /**
         * @brief Returns the resource options of the arena.
         */
        static consteval resource_options options() noexcept { return Opt; }

        /**
         * @brief Returns the growth policy of the heap chunks.
         */
        static consteval arena_growth_policy growth_policy() noexcept requires (Opt.use_upstream) {
            return GrowthPolicy;
        }
        ...
        /**
         * @brief Resets initial_buffer and the last heap chunk (if Opt.use_upstream == true) so that they
         ...
         *        from the initial buffer. If failed and Opt.use_upstream == true, it tries to allocate from the current
        ...
        noexcept(!Opt.use_upstream) {
            ...
            if constexpr (Opt.use_upstream) { // fall-back to upstream resource
                constexpr auto footer_size = sizeof(detail::chunk_footer);
                constexpr auto footer_align = alignof(detail::chunk_footer);
                ...
                auto* footer = reinterpret_cast<detail::chunk_footer*>(footer_addr);
