...
namespace urlicht::memory {
    ...
    private:
        ...
        uintptr_t ptr_{};
        UL_NO_UNIQUE_ADDRESS deleter_type deleter_{};
...
namespace std {
    ...
    // tuple_size
    template <typename T, bool IS_OWNING, typename Deleter>
    struct tuple_size<urlicht::memory::tagged_ptr<T, IS_OWNING, Deleter>>
        : std::integral_constant<std::size_t, 2> {};

    // tuple_element
    template <std::size_t I, typename T, bool IS_OWNING, typename Deleter>
    struct tuple_element<I, urlicht::memory::tagged_ptr<T, IS_OWNING, Deleter>> {
        static_assert(I < 2, "tagged_ptr tuple_element: index out of bounds");
        using type = std::conditional_t<I == 0,
                                        typename urlicht::memory::tagged_ptr<T, IS_OWNING, Deleter>::pointer,
                                        typename urlicht::memory::tagged_ptr<T, IS_OWNING, Deleter>::tag_type>;
    };
    ...
    struct pointer_traits<urlicht::memory::tagged_ptr<T, IS_OWNING, Deleter>> {
        using pointer = urlicht::memory::tagged_ptr<T, IS_OWNING, Deleter>;
