...
TEST(Arena, MoveOperations) {
    ...
    urlicht::memory::arena arena1(buffer.data(), 1024u);
...
TEST(Arena, TriesInitialFirst) {
    std::vector<char> buffer(1024);
    urlicht::memory::arena arena(buffer.data(), 1024u);
    ...
    for (size_t i = 0; i < 10; ++i) {
        [[maybe_unused]] auto alloc_res = arena.allocate(2048 * i, 2);  // All not from initial
    }
...
TEST(Arena, ResetInitialAndHead) {
    urlicht::memory::arena arena(1024u);
...
TEST(Arena, ReleaseHeapChunksOnly) {
    ...
    // External buffer
    std::vector<char> buffer(1024);
    urlicht::memory::arena arena2(buffer.data(), 1024u);
...
TEST(Arena, Comparison) {
    urlicht::memory::arena arena, arena2;
