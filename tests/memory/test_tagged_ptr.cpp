...
TEST(TaggedPtrTest, TraitsAndAlignment) {
    using Ptr = urlicht::memory::tagged_ptr<AlignedItem>;
...
TEST(TaggedPtrTest, NonOwning_ConstructionAndAccess) {
    AlignedItem item(42);
    urlicht::memory::tagged_ptr ptr(&item);
...
TEST(TaggedPtrTest, NonOwning_Modifiers) {
    AlignedItem item1(1);
    AlignedItem item2(2);
    urlicht::memory::tagged_ptr ptr(&item1);
    ptr.set_tag(5);
...
TEST(TaggedPtrTest, Owning_Lifecycle) {
    ...
    {
        auto ptr = urlicht::memory::make_tagged<AlignedItem, true>(10);
...
TEST(TaggedPtrTest, Owning_MoveSemantics) {
    ...
    // Move Constructor
    {
        auto ptr1 = urlicht::memory::make_tagged<AlignedItem, true>(50);
        ptr1.set_tag(5);
    ...
    // Move Assignment
    {
        auto ptr1 = urlicht::memory::make_tagged<AlignedItem, true>(100);
        auto ptr2 = urlicht::memory::make_tagged<AlignedItem, true>(200);
...
TEST(TaggedPtrTest, Owning_ResetAndRelease) {
    ...
    // Release
    {
        auto ptr = urlicht::memory::make_tagged<AlignedItem, true>(1);
        ptr.set_tag(2);
...
TEST(TaggedPtrTest, ArraySupport) {
    AlignedItem::reset_counts();
    {
        ...
        auto ptr = urlicht::memory::make_tagged<AlignedItem[], true>(3); // size 3
...
TEST(TaggedPtrTest, Comparisons) {
    AlignedItem a, b;
    urlicht::memory::tagged_ptr p1(&a);
    urlicht::memory::tagged_ptr p2(&a);
    urlicht::memory::tagged_ptr p3(&b);
...
TEST(TaggedPtrTest, Swap) {
    ...
    urlicht::memory::tagged_ptr p1(&a);
    p1.set_tag(1);
    urlicht::memory::tagged_ptr p2(&b);
    p2.set_tag(2);
...
TEST(TaggedPtrTest, Hashing) {
    ...
    std::hash<urlicht::memory::tagged_ptr<AlignedItem, false>> hasher;
...
TEST(TaggedPtrTest, Formatting) {
    AlignedItem item;
    urlicht::memory::tagged_ptr<AlignedItem, false> ptr(&item);
    ptr.set_tag(1);
