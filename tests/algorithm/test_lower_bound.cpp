...
TEST_F(LowerBound, EmptyRange) {
    auto it = urlicht::algorithm::lower_bound(ints.cbegin(), ints.cend(), 0);
    EXPECT_EQ(it, ints.end());
    it = urlicht::algorithm::lower_bound(ints, 9);
    EXPECT_EQ(it, ints.end());
}
...
TEST_F(LowerBound, SingleSized) {
    ints.emplace_back(1);
    // val < elem
    auto it = urlicht::algorithm::lower_bound(ints, -1);
    EXPECT_EQ(*it, 1);
    EXPECT_EQ(it, ints.begin());
    // val == elem
    it = urlicht::algorithm::lower_bound(ints, 1);
    EXPECT_EQ(*it, 1);
    EXPECT_EQ(it, ints.begin());
    // val > elem
    it = urlicht::algorithm::lower_bound(ints, 9);
...
TEST_F(LowerBound, AllElemLessOrGreater) {
    ...
    it = urlicht::algorithm::lower_bound(ints, 10);
    EXPECT_EQ(it, ints.end());
}
...
TEST_F(LowerBound, ValuePresent) {
    ...
    auto it = urlicht::algorithm::lower_bound(ints, 5);
    EXPECT_EQ(*it, 5);
    EXPECT_EQ(it, ints.begin());
    ...
    it = urlicht::algorithm::lower_bound(ints, 21);
    EXPECT_EQ(*it, 21);
...
TEST_F(LowerBound, FirstOccurrence) {
    ...
    auto it = urlicht::algorithm::lower_bound(ints, 2);
    EXPECT_EQ(*it, 2);
    EXPECT_EQ(it, ints.begin() + 2);
}
...
TEST_F(LowerBound, AbsentBetweenElem) {
    ...
    auto it = urlicht::algorithm::lower_bound(ints, 6);
    EXPECT_EQ(*it, 7);
    EXPECT_EQ(it, ints.begin() + 2);

    for (int i = 11; i < 15; ++i) {
        it = urlicht::algorithm::lower_bound(ints, i);
        EXPECT_EQ(*it, 15);
...
TEST_F(LowerBound, OnSubrange) {
    ...
    auto it = urlicht::algorithm::lower_bound(sr, 2);
    EXPECT_EQ(*it, 4);
    EXPECT_EQ(it, ints.begin() + 2);

    it = urlicht::algorithm::lower_bound(sr, 5);
    EXPECT_EQ(*it, 6);
    EXPECT_EQ(it, ints.begin() + 3);

    it = urlicht::algorithm::lower_bound(sr, 11);
    EXPECT_EQ(it, end);
}
...
TEST_F(LowerBound, CustomComparator) {
    ...
    auto it = urlicht::algorithm::lower_bound(ints, 17, std::greater{});
    EXPECT_EQ(*it, 17);
    EXPECT_EQ(it, ints.begin() + 1);

    it = urlicht::algorithm::lower_bound(ints, 16, std::greater{});
    EXPECT_EQ(*it, 15);
    EXPECT_EQ(it, ints.begin() + 3);

    it = urlicht::algorithm::lower_bound(ints, 9, std::greater{});
    EXPECT_EQ(it, ints.end());
}
...
TEST_F(LowerBound, CustomProjection) {
    ...
    auto it = urlicht::algorithm::lower_bound(aggregates, 3, std::less{}, Proj{});
    EXPECT_EQ(it->id, 3);
    EXPECT_EQ(it, aggregates.begin() + 1);

    it = urlicht::algorithm::lower_bound(aggregates, 4, std::less{}, Proj{});
    EXPECT_EQ(it->id, 8);
    EXPECT_EQ(it, aggregates.begin() + 3);

    it = urlicht::algorithm::lower_bound(aggregates, 16, std::less{}, Proj{});
    EXPECT_EQ(it, aggregates.end());
}

TEST_F(LowerBound, AlternativeContainers) {
    auto test_exist = [](auto& cont, auto val, size_t pos) {
        auto it = urlicht::algorithm::lower_bound(cont, val);
    ...
    urlicht::container::inplace_vector<std::string, 10> iv{
        "a", "b", "b", "c", "e", "f"
    };
    // array
    test_exist(arr, 5, 1);
...
TEST_F(LowerBound, MatchesSTDOverRandomInputs) {
    ...
    for (int trial = 0; trial < 500; ++trial) {
        ...
        for (int q = -2050; q <= 2050; ++q) {
            const auto it_u = urlicht::algorithm::lower_bound(v, q);
