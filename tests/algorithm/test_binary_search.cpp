...
TEST(BinarySearch, Projection) {
    ...
    EXPECT_TRUE(urlicht::binary_search(v.begin(), v.end(), 2, std::less<>{}, &Record::key));
    EXPECT_TRUE(urlicht::binary_search(v, 8, std::less<>{}, &Record::key));
    EXPECT_FALSE(urlicht::binary_search(v, 3, std::less<>{}, &Record::key));
}
