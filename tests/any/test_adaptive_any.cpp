...
TEST(AdaptiveAnyTest, InplaceConstruction) {
    ...
    adaptive_any<16, 8> any(urlicht::inplace<Aligned32>, Aligned32{1, 2, 3, 4});
    EXPECT_TRUE(any.has_value());
    EXPECT_EQ(any_cast<const Aligned32&>(any), Aligned32(1, 2, 3, 4));

    adaptive_any<16, 8> any2(urlicht::inplace<Aligned32&&>, Aligned32{1, 2, 3, 4});
    EXPECT_TRUE(any.has_value());
    EXPECT_EQ(any_cast<Aligned32>(any), Aligned32(1, 2, 3, 4));

    adaptive_any<16, 8> any3(urlicht::inplace<const Aligned32&>, Aligned32{1, 2, 3, 4});
    EXPECT_TRUE(any.has_value());
    EXPECT_EQ(any_cast<Aligned32&&>(any), Aligned32(1, 2, 3, 4));
}
