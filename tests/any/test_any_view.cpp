...
TEST_F(AnyViewTest, Basics) {
    ...
    // Check value
    EXPECT_EQ(urlicht::any::any_cast<int>(v), 42);
}

TEST_F(AnyViewTest, PointerConstruction) {
    double d = 3.14159;
    urlicht::any::any_view v(&d);

    EXPECT_TRUE(v.is<double>());
    EXPECT_DOUBLE_EQ(urlicht::any::any_cast<double>(v), 3.14159);

    int* p = nullptr;
    urlicht::any::any_view null_int(p);
    EXPECT_FALSE(null_int.has_value());
    EXPECT_EQ(null_int.type_info(), typeid(void));

    urlicht::any::any_view null_ptr(nullptr);
...
TEST_F(AnyViewTest, ModifyOriginal) {
    ...
    EXPECT_EQ(urlicht::any::any_cast<int>(v), 10);

    i = 20;
    EXPECT_EQ(urlicht::any::any_cast<int>(v), 20);
}
...
TEST_F(AnyViewTest, BadAnyCast) {
    ...
    EXPECT_THROW(urlicht::any::any_cast<float>(v), std::bad_any_cast);
    EXPECT_EQ(urlicht::any::any_cast<float>(&v), nullptr);

    // Redundant qualifiers (ignored)
    EXPECT_EQ(urlicht::any::any_cast<const int&>(v), 42);
    EXPECT_EQ(urlicht::any::any_cast<int&&>(v), 42);
}
...
TEST_F(AnyViewTest, Swap) {
    ...
    urlicht::any::any_view v1(a);
    urlicht::any::any_view v2(b);
...
TEST_F(AnyViewTest, MakeAnyViewHelper) {
    std::string s = "Hello";
    auto v = urlicht::any::make_any_view(s);

    EXPECT_TRUE(v.is<std::string>());
    EXPECT_EQ(urlicht::any::any_cast<std::string>(v), "Hello");
}

TEST_F(AnyViewTest, ContainerUsage) {
    std::vector<urlicht::any::any_view> views;
