...
TEST(StaticShardedCounter, StaticMethods) {
    using C = static_sharded_counter<std::int64_t, 17, 9>;
    static_assert(C::threshold() == 17);
    static_assert(C::num_local_cnt() == 9);
    SUCCEED();
}

TEST(StaticShardedCounter, DefaultInit) {
    static_sharded_counter<std::int64_t, 8, 2> c;
    EXPECT_EQ(c.get_exact(), 0);
    EXPECT_EQ(c.get_approximate(), 0);
}

TEST(StaticShardedCounter, InitWithValue) {
    static_sharded_counter<std::int64_t, 8, 2> c(123);
    EXPECT_EQ(c.get_approximate(), 123);
    EXPECT_EQ(c.get_exact(), 123);
}

TEST(StaticShardedCounter, IncrementLocal) {
    using C = static_sharded_counter<std::int64_t, 1000, 1>;
    C c;
    c.increment();
    c.increment(4);
    EXPECT_EQ(c.get_approximate(), 0);
...
TEST(StaticShardedCounter, FlushOnThresholdUnsigned) {
    static_sharded_counter<std::uint64_t, 4, 1> c;
    c.increment();
    c.increment();
    c.increment();
...
TEST(StaticShardedCounter, FlushOnNegativeThresholdSigned) {
    using C = static_sharded_counter<std::int64_t, 3, 1>;
    C c;
    c.increment(-1);
    c.increment(-1);
    EXPECT_EQ(c.get_approximate(), 0);
...
TEST(StaticShardedCounter, FlushLocal) {
    static_sharded_counter<std::int64_t, 1000, 1> c;
    c.increment(7);
    EXPECT_EQ(c.get_approximate(), 0);
    c.flush_local();
...
TEST(StaticShardedCounter, FlushAll) {
    static_sharded_counter<std::int64_t, 1000, 4> c;
...
TEST(StaticShardedCounter, PredicateFalse) {
    static_sharded_counter<std::int64_t, 1000, 1> c;
    c.increment(10);
...
TEST(StaticShardedCounter, PredicateTrue) {
    static_sharded_counter<std::int64_t, 1000, 1> c;
    c.increment(10);
...
TEST(StaticShardedCounter, SetNewValue) {
    static_sharded_counter<std::int64_t, 1000, 1> c;
    c.increment(999);
    EXPECT_EQ(c.get_approximate(), 0);
    c.set(42);
...
TEST(StaticShardedCounter, ConstGetExactNoFlush) {
    using C = static_sharded_counter<std::int64_t, 1000, 1>;
    C c;
    c.increment(15);
    const C& cc = c;
...
TEST(StaticShardedCounter, HighlyMultithreaded) {
    static_sharded_counter<std::int64_t, 256, 8> c;
...
TEST(StaticShardedCounter, MixedSignedDiffs) {
    static_sharded_counter<std::int64_t, 64, 8> c;
...
TEST(StaticShardedCounter, ConcurrentReadersAndWriters) {
    using C = static_sharded_counter<std::int64_t, 128, 8>;
    C c;
    ...
    for (std::size_t i = 0; i < readers; ++i) {
        ts.emplace_back([&] {
            ...
            while (!done.load(std::memory_order_acquire)) {
                (void)c.get_approximate();
                (void)static_cast<const C&>(c).get_exact();
            }
        });
    }
